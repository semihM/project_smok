...
function ScriptMode_StringReplace(string, original, replacement)
{
...
function ScriptMode_Init( modename, mapname )
{
	...
	if ( IncludeScript( "map_" + mapname, g_MapScript ) )  // include map specific script
	{
	...
	// include external files for mods
	local mods = split(FileToString("mods.txt"), "\r\n");
	
	foreach (mod in mods)
	{
		if ( mod.find("//") != null )
		{
			mod = ScriptMode_StringReplace(mod, "//" + ".*", "");
			mod = rstrip(mod);
		}
		IncludeScript("Mods/" + mod);
	}
